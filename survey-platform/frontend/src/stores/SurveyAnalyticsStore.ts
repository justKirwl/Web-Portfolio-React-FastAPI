...
const store: StateCreator<AnalyticsState> = ((set, get) => ({
    ...
    getDayData: () => {
        const responses = useSurveyStore.getState().surveyData.responses.length
        const views = useSurveyStore.getState().surveyData.views
...
export const useSurveyAnalyticsStore = create<AnalyticsState>()(store)