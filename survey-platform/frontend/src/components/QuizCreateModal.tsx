import { X, Plus, Trash2, GripVertical, CheckCircle, Trophy, BarChart3, AlertCircle, Globe, Target, BookOpen, Award, Lightbulb } from 'lucide-react';
import { useQuizCreateStore } from '../stores/QuizCreateStore';
import { useDashboardStore } from '../stores/DashboardStore';
import { useTranslation } from '../../node_modules/react-i18next';
import { difficulties, languages } from '../utils/surveyCreateTypes';

export default function CreateQuizModal() {
  const { currentQuestion, quizData, setCurrentQuestion, setQuizData, createQuiz, isLoading, topicInput, learningInput, requirementInput, errors, setErrors, setLearningInput, setRequirementInput, setTopicInput } = useQuizCreateStore() 
  const { setQuizVisible: onClose } = useDashboardStore()
  ...
  const addQuestion = () => {
    ...
    if (!currentQuestion.question.trim()) {
      newErrors.currentQuestion = t('quizCreate.addQuestionError.currentQuestion');
    }

    const filledOptions = currentQuestion.options.filter(opt => opt.trim());
    if (filledOptions.length < 2) {
      newErrors.currentOptions = t('quizCreate.addQuestionError.currentOptions');
    }
  ...
  const validateAndCreate = () => {
    ...
    if (!quizData.title.trim()) {
      newErrors.title = t('quizCreate.validateErrors.title');
    }

    if (quizData.questions.length === 0) {
      newErrors.questions = t('quizCreate.validateErrors.questions');
    }

    if (!quizData.language) {
      newErrors.language = t('quizCreate.validateErrors.language');
    }

    if (!quizData.difficulty) {
      newErrors.difficulty = t('quizCreate.validateErrors.difficulty');
    }
      ...
      <div className="bg-[var(--color-base-200)] border-2 border-[var(--color-base-300)] rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[var(--color-base-300)]">
          <div className='flex items-center gap-4'>
            <div>
              <BarChart3 className="w-10 h-10 text-[var(--color-secondary)]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-[var(--color-base-content)]">{t('quizCreate.title')}</h2>
              <p className="text-sm text-[var(--colo, Br-base-content)] opacity-60 mt-1">{t('quizCreate.desc')}</p>
            </div>  
          </div>
          <button onClick={() => onClose(false)} className="cursor-pointer p-2 rounded-lg hover:bg-[var(--color-base-300)] transition-all">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  <option value="">{t('quizCreate.selectLanguage')}</option>
                  {languages.map(lang => (
              ...
              <div>
                ...
                >
                  <option value="">{t('quizCreate.selectDifficulty')}</option>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t('quizCreate.topicsPlaceholder')}
              ...
              <p className="text-xs text-[var(--color-base-content)] opacity-60 mt-2">
                {t('quizCreate.topicsAdded', { count: quizData.topics.length || 0 })}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t('quizCreate.learningsPlaceholder')}
              ...
              <p className="text-xs text-[var(--color-base-content)] opacity-60 mt-2">
                {t('quizCreate.learningsAdded', { count: quizData.learnings.length || 0 })}
              </p>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium mb-2 text-[var(--color-base-content)] flex items-center gap-2">
                ...
                {t('quizCreate.requirementsCovered')}
              </label>
              <div className="flex gap-2">
                <input
                  type="text"
                  ...
                  placeholder={t('quizCreate.requirementsPlaceholder')}
              ...
              <p className="text-xs text-[var(--color-base-content)] opacity-60 mt-2">
                {t('quizCreate.requirementsAdded', { count: quizData.requirements.length || 0 })}
              </p>
            </div>
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm font-bold text-[var(--color-base-content)] opacity-60">{t('quizCreate.q')}{index + 1}</span>
                          ...
                          {q.options.filter(opt => opt.trim()).map((opt, i) => (
                            <div
                              ...
                              className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                                i === q.correctAnswer
                                  ? 'bg-[var(--color-base-200)] bg-opacity-20 border border-[var(--color-success)]'
          ...
          <div className="space-y-4 border-2 border-dashed border-[var(--color-base-300)] rounded-lg p-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder={t('quizCreate.textPlaceholder')}
                className={`w-full px-4 py-3 rounded-lg bg-[var(--color-base-300)] text-[var(--color-base-content)] border-2 ${
        ...
        <div className="flex items-center justify-between gap-3 p-6 border-t border-[var(--color-base-300)]">
          <div className="text-sm text-[var(--color-base-content)] opacity-60">
            <p>{quizData.questions.length} {quizData.questions.length > 1 ? quizData.questions.length > 2 && language === 'ru' ? t('quizCreate.questionsAdded') : t('quizCreate.questions').toLowerCase() : t('quizCreate.question')} • {totalPoints} {t('quizCreate.totalPoints')}</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="cursor-pointer px-6 py-2 rounded-lg bg-[var(--color-primary)] text-[var(--color-primary-content)] hover:scale-105 transition-all font-medium"
            >
              {!isLoading ? t('quizCreate.createQuiz') : <div className='loading'></div>}
            </button>
          </div>
        </div>
      </div>
