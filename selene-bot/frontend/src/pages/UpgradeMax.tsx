...
import { useUpgradeMaxActions, useUpgradeMaxCardData, useUpgradeMaxErrors, useUpgradeMaxInfo, useUpgradeMaxTouched } from "../stores/UpgradeMaxStore"
import { InfoIcon, SelectChevronDown, UpgradeCheckMark } from "../components/Icons"
import useUpgradeMaxFunctions from "../utils/upgradeMaxFunctions"
import { useTranslation } from "../../node_modules/react-i18next"

export default function UpgradeMax() {
  const formData = useUpgradeMaxCardData()
  const errors = useUpgradeMaxErrors()
  const touched = useUpgradeMaxTouched()

  const { currentPricing, handleFieldBlur, handleFieldChange, handlePurchase, formatCardNumber, formatExpiryDate, formatZipCode, annualTotal, pricing } = useUpgradeMaxFunctions()

  const { isProcessing, usage } = useUpgradeMaxInfo()
  const { setUsage } = useUpgradeMaxActions()
  ...
  const includedInMax = t('upgradeMax.features', { returnObjects: true }) as string[]
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-3">
                <button
                  ...
                  style={{
                    background: usage === "5x" ? "var(--color-base-200)" : "var(--color-base-300)",
                    border: `2px solid ${usage === "5x" ? "var(--color-outline)" : "var(--color-outline-2)"}`,
                  }}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{
                          borderColor: usage === "5x" ? "var(--color-outline)" : "var(--color-outline-2)",
                        }}
                      >
                        {usage === "5x" && (
                          <div
                      ...
                      <div>
                        <p className="font-medium" style={{ color: "var(--color-base-content)" }}>
                          {t('upgradeMax.usageOptions.fiveX')}
                        </p>
                        <p className="text-xs" style={{ color: "var(--color-base-text)", opacity: 0.5 }}>
                          <span className="raleway">$</span>{t('upgradePro.monthlySubtext', { price: pricing['5x'].priceWithTax })}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <button
                  onClick={() => setUsage("20x")}
                  className="inter w-full p-4 rounded-xl text-left transition-all relative overflow-hidden"
                  style={{
                    background: usage === "20x" ? "var(--color-base-200)" : "var(--color-base-300)",
                    border: `2px solid ${usage === "20x" ? "var(--color-outline)" : "var(--color-outline-2)"}`,
                  }}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                        style={{
                          borderColor: usage === "20x" ? "var(--color-outline)" : "var(--color-outline-2)",
                        }}
                      >
                        {usage === "20x" && (
                          <div
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-medium" style={{ color: "var(--color-base-content)" }}>
                            {t('upgradeMax.usageOptions.twentyX')}
                          </p>
                          <div
                            className={`text-xs px-${language === 'ru' ? '4' : '2'} py-${language === 'ru' ? '1.5' : '0.5'} bg-[var(--color-outline)]/20 rounded-full font-semibold ${language === 'ru' ? 'whitespace-nowrap' : ''}`}
                            style={{
                        ...
                        <p className="text-xs" style={{ color: "var(--color-base-text)", opacity: 0.5 }}>
                          <span className="raleway">$</span>{t('upgradePro.yearlySubtext', { price: pricing['20x'].priceWithTax })}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            >
              <p
                ...
                style={{ color: "var(--color-base-content)" }}
              >
                {t('upgradeMax.includedTitle')}
              </p>
              <ul className="space-y-3">
                {includedInMax.map((feature, index) => (
                  <li
                    key={index}
            ...
            >
              ...
              <div className="space-y-3 mb-4 pb-4 border-b" style={{ borderColor: "var(--color-base-300)" }}>
                <div className="flex justify-between text-sm">
                  <span style={{ color: "var(--color-base-text)", opacity: 0.7 }}>
                    {t('upgradeMax.maxPlan', { usage: usage })}
                  </span>
                ...
                {usage === "20x" && (
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "var(--color-primary)" }}>
                      {t('upgradeMax.annualDiscount')}
                    </span>
                    <span style={{ color: "var(--color-primary)" }}>
                      -<span className="raleway">$</span>{(currentPricing.price / 2).toFixed(2)}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-between items-baseline mb-2">
                <span className="text-lg font-semibold" style={{ color: "var(--color-base-text)" }}>
                  {t('upgradeMax.monthlyTotal')}
                </span>
                <div className="text-right">
                  <p className="text-2xl font-bold" style={{ color: "var(--color-base-text)" }}>
                    <span className="raleway">$</span>{usage === "20x" ? (annualTotal || 0).toFixed(2) : currentPricing.priceWithTax}
                  </p>
                  {usage === "20x" && (
                    <p className="text-xs" style={{ color: "var(--color-base-text)", opacity: 0.5 }}>
                      {t('upgradeMax.billedAnnually')}
                    </p>
                  )}
                </div>
              </div>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: "var(--color-base-content)" }}>
                  {t('upgradeMax.emailAddress')}
                </label>
                <input
                  type="email"
                  value={formData.email}
                  onChange={(e) => handleFieldChange("email", e.target.value)}
                  placeholder={t('upgradeMax.emailPlaceholder')}
                  className={`w-full px-4 py-3 rounded-lg outline-none text-sm transition-all duration-200 focus:shadow-xs focus:shadow-blue-300`}
                  style={{
                    background: "var(--color-base-300)",
                    border: `1px solid ${
              ...
              >
                {isProcessing ? (
                  <span className="flex items-center justify-center gap-2">
                    ...
                    {t('upgradeMax.processing')}
                  </span>
                ) : (
                  <span>{t('upgradeMax.purchaseTitle')}<span className="raleway">$</span>{usage === "20x" ? (annualTotal || 0).toFixed(2) : currentPricing.priceWithTax.toFixed(2)}</span>
                )}
              </button>
              ...
              >
                ...
                {usage === "20x" && t('upgradePro.renewAnnually')}
              </p>
            </div>
          </div>
          </div>
