...
export default function ProtectPage({ children, location }: { children: JSX.Element, location: string }) {
    ...
    if (checkingAuth || !isFetched.current) return (
        <div className="flex items-center justify-center h-screen bg-[var(--color-base-100)]">
            <div className="loader w-20 h-20 border-4 border-teal-500 border-t-transparent rounded-full animate-spin"></div>
        </div>
    )

    if (!isAuthorized) return <Navigate to={`/auth?next=${location}`} replace />;
