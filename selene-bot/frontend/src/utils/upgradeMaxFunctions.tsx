...
import { useUpgradeProActions, type CardData } from "../stores/UpgradeProStore"
import { useTranslation } from "../../node_modules/react-i18next"

export default function useUpgradeMaxFunctions() {
    const formData = useUpgradeMaxCardData()
    const errors = useUpgradeMaxErrors()
    const touched = useUpgradeMaxTouched()

    const { usage, isProcessing } = useUpgradeMaxInfo()
    const { setFormData, setErrors, setTouched, setIsProcessing } = useUpgradeMaxActions()

    const { upgradePlan } = useUpgradeProActions()
    ...
    const pricing = {
        '5x': { price: 120, priceWithTax: 135.4, discount: 0 },
        '20x': { price: 200, priceWithTax: 220.89, discount: 50 },
    };

    const currentPricing = pricing[usage];
    const annualTotal = usage === "20x" ? currentPricing.priceWithTax : null;
    ...
    const handlePurchase = useCallback(async () => {
        ...
        setTimeout(async () => {
            const res = await upgradePlan('max');
